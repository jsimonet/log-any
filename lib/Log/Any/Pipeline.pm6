...
=head2 get-available-adapters
	...
	the filter.
=end pod
	method !get-available-adapters( :$msg, :$severity, :$category, :%tags ) {

		return gather for @!adapters -> %elem {
			# Filter : check if the adapter meets the requirements
			with %elem{'filter'} {
				next unless %elem{'filter'}.filter( :$msg, :$severity, :$category, :%tags );
			}
	...
	method dispatch( DateTime :$date-time!, :$msg!, :$severity!, :$category!, :%tags ) {
		...
		if $!asynchronous {
			...
			# note "sync dispatch";
			return self!dispatch-synchronous( :$date-time, :$msg, :$severity, :$category, :%tags );
		}
	}

	method !dispatch-synchronous( :$date-time!, :$msg! is copy, :$severity!, :$category!, :%tags ) {
		for self!get-available-adapters(  :$msg, :$severity, :$category ) -> %elem {
			if %elem {
				...
				my $msgToHandle = $msg;
				if %elem{'formatter'} {
					$msgToHandle = %elem{'formatter'}.format( :$date-time, :$msg, :$category, :$severity, :%tags );
				}
	...
	method will-dispatch( :$severity, :$category, :%tags ) returns Bool {
		return self!get-available-adapters( :$severity, :$category. :%tags ).so;
	}
